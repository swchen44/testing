...
**文件版本**：v3.6
**狀態**：Draft
**依據**：agents-requirements.md v3.6
...
| Unit | `TestCollectBaseExperts` | `collect_base_experts()` DFS 遍歷、去重、遞迴 | 6 |
...
`--add` 不加旗標時，CLAUDE.md 包含 identity 的四份文件，加上所有 Base Experts 的四份文件（`## Base Experts` 區段）：
...
@connsys-jarvis/framework/framework-base-expert/expert.md
@connsys-jarvis/wifi-bora/wifi-bora-base-expert/soul.md
@connsys-jarvis/wifi-bora/wifi-bora-base-expert/rules.md
@connsys-jarvis/wifi-bora/wifi-bora-base-expert/duties.md
...
加上 `--with-all-experts` 旗標時，CLAUDE.md 以 **Identity + Base Experts + Capabilities 三區段**呈現：

- Base Experts 在 `## Base Experts` 區段完整輸出四份文件
- Capabilities 區段只列非 Base Expert 的 `expert.md`（避免 Base Expert 重複出現）
...
**規則**：任何 `expert.json` 中 `is_base=true` 的 Expert（不論是否為 identity），其四份文件必須寫入 CLAUDE.md 的 `## Base Experts` 區段。

**偵測範圍**：DFS 遍歷整個依賴樹
- 從每個已安裝 Expert 出發
- 遞迴讀取 `expert.json.dependencies`
...
```
CLAUDE.md 包含：A 的四份文件（主區段） + B 的四份文件（Base Experts）+ C 的四份文件（Base Experts）
D 不輸出（is_base=false）

**實作函式**：`collect_base_experts(workspace, installed)` → `list[Path]`
